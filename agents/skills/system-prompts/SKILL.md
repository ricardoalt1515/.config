---
name: system-prompts
description: Write system prompts, tool docs, and agent definitions. Dense, imperative, RFC-keyed. Use when authoring or editing any prompt the model reads — AGENTS.md, SKILL.md, prompt templates, subagent instructions, or tool descriptions.
---

# System Prompts

House style for prompts. Dense, imperative, RFC-keyed.
...
**Aliases**: prefer `NEVER` over `MUST NOT` and `AVOID` over `SHOULD NOT`. Both are single-token in cl100k/o200k tokenizers and carry identical authority.
...
Tool prompts teach the agent **when to reach for the tool, what shape its inputs take, and which failure modes are the agent's responsibility**. Everything else stays in code.
...
- If the agent's behavior shouldn't change based on a detail, the detail does NOT belong in the prompt.
- Each sentence MUST shift a decision the agent makes.

### Anatomy of a good tool prompt

1. **One-line purpose.** What problem it solves, in the agent's vocabulary.
...
4. **Failure shapes the agent owns.** Things the agent can fix by changing its input. Skip failures the engine recovers from silently.
...
- Performance characteristics unless they change the agent's strategy.
- Telemetry, logging, debug flags, env vars the agent cannot set.
- Version history, deprecated parameters.
...
- Examples MUST be runnable shape, not pseudo-code.
